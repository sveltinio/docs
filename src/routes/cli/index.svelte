<script lang="ts" context="module">
   ...
   export const load: Load = async ({ fetch }) => {
      ...
      if (res.ok) {
         ...
         const cli = schemaValidator.validate<Array<ContentItem>>(
            ContentItemSchema,
            data
         );
         return {
...
<section class="artifact-container">
	<div class="content">
      ...
      <ul>
      {#each cli as item }
         <li><a sveltekit:prefetch href={`${base}/cli/${item.slug}`}>{item.title}</a></li>
      {/each}
      </ul>
      {:else}
         <h2 class="message warning">
            Nothing to show here! Create some content first and reload the page:
            <span><pre><code class="text-default">sveltin new content cli/getting-started</code></pre></span>
         </h2>
      {/if}
   </div>
</section>
